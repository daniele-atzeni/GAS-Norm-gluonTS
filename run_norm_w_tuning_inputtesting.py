...
class Objective:
    ...
    def __init__( self, MODEL, DATASET_NAME, ctx, DATASET_FILE_FOLDER, multivariate=False, mean_str=0.1, var_str=0.1):
        ...
        self.train_original = [self.train_original[0]]
    ...
    def train_and_test(self, params, save=False):
        ...
        if self.model == 'feedforward' and self.multivariate:
          ...
          from my_models.gluonts_models.univariate.probabilistic_forecast.transformer_default._estimator import (
                TransformerEstimator as Transformer_default,
            )
        ...
        #   )
          estimator = Transformer_test( # Transformer_gluonts_gas_means Transformer_gluonts_gas_means_point Transformer_test
              ...
              scaling=False,
              dropout_rate= params['dropout_rate'],
              trainer=Trainer(hybridize=False,ctx=self.ctx,epochs=1, learning_rate=params['trainer:learning_rate'],
                                     num_batches_per_epoch=100, callbacks=[history]),
          )
