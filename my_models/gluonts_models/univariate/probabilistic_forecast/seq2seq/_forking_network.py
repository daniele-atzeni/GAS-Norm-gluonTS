...
class ForkingSeq2SeqNetworkBase(gluon.HybridBlock):
    ...
    def get_decoder_network_output(
        ...
        scaled_past_target, scale = self.scaler(past_target, past_observed_values)
        ...
        dec_input_static, dec_input_dynamic = self.enc2dec(
            ...
            enc_output_dynamic.slice_axis(axis=1, begin=-self.num_forking, end=None),
...
class ForkingSeq2SeqTrainingNetwork(ForkingSeq2SeqNetworkBase):
    def hybrid_forward(
        ...
        # only distribution output for us
        """
        ...
        distr = self.distr_output.distribution(
            new_distr_args, scale=scale.expand_dims(axis=1)
        )
...
class ForkingSeq2SeqPredictionNetwork(ForkingSeq2SeqNetworkBase):
    def hybrid_forward(
        ...
        fcst_output = F.squeeze(
            self.quantile_proj(F.slice_axis(dec_output, axis=1, begin=-1, end=None)),
            axis=1,
        )
...
class ForkingSeq2SeqDistributionPredictionNetwork(ForkingSeq2SeqNetworkBase):
    def hybrid_forward(
        ...
        return new_distr_args, loc, scale
