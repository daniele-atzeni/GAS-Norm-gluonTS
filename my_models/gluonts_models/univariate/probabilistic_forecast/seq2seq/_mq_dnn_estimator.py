...
class MQCNNEstimator(ForkingSeq2SeqEstimator):
    ...
    def __init__(
        ...
        assert prediction_length > 0, f"Invalid prediction length: {prediction_length}."
        ...
        self.channels_seq = channels_seq if channels_seq is not None else [30, 30, 30]
        self.dilation_seq = dilation_seq if dilation_seq is not None else [1, 3, 9]
    ...
    def from_inputs(cls, train_iter, **params):
        ...
        logger.info(f"gluonts[from_inputs]: User supplied params set to {params}")
