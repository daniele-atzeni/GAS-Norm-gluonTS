...
class WaveNetEstimator(GluonEstimator):
    ...
    estimator = Wavenet_gluonts_gas_means(
                    mean_layer,                                 OK
                    freq=frequency,                             OK
                    distr_output=StudentTOutput(),            NO
                    prediction_length=prediction_length,        OK
                    context_length=context_length,            NO
                    trainer=trainer,                            OK
                    **estimator_parameters,                     OK
                )
    """
    ...
    def __init__(
        ...
        goal_receptive_length = max(2 * seasonality, 2 * self.prediction_length)
        if dilation_depth is None:
            d = 1
            while (
                WaveNet.get_receptive_field(dilation_depth=d, n_stacks=n_stacks)
