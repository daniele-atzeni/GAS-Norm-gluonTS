...
class DeepARTrainingNetwork(DeepARNetwork):
    ...
    def distribution(
        ...
        loc = F.mean(past_target, axis=1).expand_dims(axis=1)
        diff = F.broadcast_sub(past_target, loc)
        var = F.mean(diff * diff, axis=1).expand_dims(axis=1)
        scale = F.sqrt(var)
        scaled_target = F.broadcast_div(diff, (scale + 1e-8))
        scaled_future_target = F.broadcast_div(F.broadcast_sub(future_target, loc), (scale + 1e-8))
        # unroll the decoder in "training mode"
        # i.e. by providing future data as well
        rnn_outputs, _, _, _, _ = self.unroll_encoder(
            F=F,
            feat_static_cat=feat_static_cat,
            feat_static_real=feat_static_real,
            past_time_feat=past_time_feat,
            past_target=scaled_target,
        ...
        return (
            (
                self.distr_output.distribution(distr_args, scale=scale, loc=loc),
                rnn_outputs,
            )
            if return_rnn_outputs
            else self.distr_output.distribution(distr_args, scale=scale, loc=loc)
        )
...
class DeepARPredictionNetwork(DeepARNetwork):
    ...
    def sampling_decoder(
        ...
        repeated_loc = loc.repeat(repeats=self.num_parallel_samples, axis=0)
        repeated_states = [
        ...
        for k in range(self.prediction_length):
            ...
            distr = self.distr_output.distribution(
                distr_args, scale=repeated_scale, loc=loc
            )
            ...
            # here I have to add normalized predictions
            new_samples_ar = (new_samples - repeated_loc) / repeated_scale
            repeated_past_target = F.concat(
                repeated_past_target, new_samples_ar, dim=1
            )
    ...
    def hybrid_forward(
        ...
        _, state, _, static_feat, imputed_sequence = self.unroll_encoder(
            F=F,
            feat_static_cat=feat_static_cat,
            feat_static_real=feat_static_real,
            past_time_feat=past_time_feat,
