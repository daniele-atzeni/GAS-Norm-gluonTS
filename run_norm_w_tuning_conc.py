        ...
        #   )
          ...
          estimator = DeepAR_gluonts_univariate_gas(
            ...
            #   num_layers= params['num_layers'],
              trainer=Trainer(hybridize=False,ctx=self.ctx,epochs=5, learning_rate=params['trainer:learning_rate'],
                                     num_batches_per_epoch=100, callbacks=[history]),
...
def run_study(mean_str, var_str, model_choice, DATASET_NAME, ctx, DATASET_FILE_FOLDER, multivariate, n_trials):
...
def run(DATASET_NAME, model_choice, ctx, DATASET_FILE_FOLDER, n_trials, mean_str, var_str):
    ...
    with Pool() as pool:
        args = [(mean_str, var_str, model_choice, DATASET_NAME, ctx, DATASET_FILE_FOLDER, multivariate, n_trials) for mean_str, var_str in zip(mean_strs, var_strs)]
        print(args)
        raise ValueError("STOP")
        results = pool.starmap(run_study, args)

    for study, mean_str, var_str in zip(results, mean_strs, var_strs):
